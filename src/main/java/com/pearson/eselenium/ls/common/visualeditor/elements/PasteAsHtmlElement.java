package com.pearson.eselenium.ls.common.visualeditor.elements;

import com.pearson.eselenium.ls.common.pages.utils.SwitchToFrame;
import com.pearson.eselenium.ls.common.radeditor.pages.PasteAsHtmlPage;
import com.pearson.test.eselenium.framework.BasicPieceObject;
import com.pearson.test.eselenium.framework.core.UIElement;

/**
 * This class identifies the Element and Actions / Methods for that Element.
 * 
 * @author Kapil.Aare
 */
public class PasteAsHtmlElement extends BasicPieceObject {

    SwitchToFrame switchToFrame = new SwitchToFrame(uiDriver);
    private long waitTime=Long.parseLong(config.getValue("waitTime"));

	/*Constructor*/
	public PasteAsHtmlElement(UIElement element) {
        super(element);
    }

    /**
     * Method helps to click Paste As Html Button
     * 
     * @author soundarya
     * @return PasteAsHtmlpage
     * @throws Exception
     */
    public PasteAsHtmlPage clickPasteAsHtmlButton() throws Exception {
        try {
            //switchToFrame.switchToFrameContent();
            logInstruction("LOG INSTRUCTION: CLICK Paste As Html Button");
            root.clickAndWait(waitTime);
        } catch (Exception e) {
            throw new Exception("UNABLE TO CLICK Paste As Html Button \n METHOD: clickPasteAsHtmlButton" + e
            		.getLocalizedMessage());
        }
        return new PasteAsHtmlPage(uiDriver);
    }
}