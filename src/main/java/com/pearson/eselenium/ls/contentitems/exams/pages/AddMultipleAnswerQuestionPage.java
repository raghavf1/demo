...
public class AddMultipleAnswerQuestionPage extends BasePageObject {
	...
	private UIElement radOption = createElement(UIType.Css, "#RADcorrect{1}");
	private String alertMsg;
	  private UIElement lnkEditorSwitch= createElement(UIType.Css, "#telerikRAD");
	    boolean flag=false;
	  
	/* Constructor */
	public AddMultipleAnswerQuestionPage(UIDriver driver) {
		super(driver);
		setResponsesCount();
		...
		for (int i = 0; i < responsesCount; i++) {
			...
			responseRadListArray.add(new VisualEditor(uiDriver,
					responseRadToolId[i], responseRadFrameId[i],
					responseRadFooterId[i]));
		}
		...
		logInstruction("LOG INSTRUCTION: ## Add Multiple Answer Question ##");
	}
	 ...
	 * @return AddMultipleAnswerQuestionPage
	 * @throws Exception
	 */
	public AddMultipleAnswerQuestionPage enterPoints(String pointsToBeEntered)
			throws Exception {
		try {
			logInstruction("LOG INSTRUCTION: ENTERING THE POINTS FOR MULTIPLE ANSWER QUESTION");
			frameSwitch.switchToFrameContent();
			uiDriver.waitToBeDisplayed(txtPoints, waitTime);
			txtPoints.clearAndSendKeys(pointsToBeEntered);
		} catch (Exception e) {
			throw new Exception(
					"ISSUE IN ENTERING THE POINTS FOR MULTIPLE ANSWER QUESTION."
							+ "\n METHOD : enterPoints \n"
							+ e.getLocalizedMessage());
		}
		return this;
	 ...
	 * @return ExamHomePage
	 */
	public ExamHomePage addMultipleAnswerQuestion(String question,
